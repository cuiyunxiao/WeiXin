        ...
        if (res.data.status == 200) {
          wx.request({
            ...
            success: function (res) {
              ...
              setTimeout(function () {
                wx.redirectTo({
                  url: '../entityorder_success/entityorder_success' + '?orderNo=' + orderNo + '&tel=' + tel,
                })
              }, 2500)
            }
          })
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        that.setData({
          ...
          orderdets: res.data.data.orderdets,
        })
        var adderss = res.data.data.address;
        if (Object.keys(adderss).length == 0 && app.appData.chooseflag == 0) {
          wx.request({
            url: geturl(findMrAddress),
            header: {
